...
TypeUI UI; //声明系统交互结构体
TypePI OutLoopPI,CurrentPI; //声明电压电流控制PI
...
void ControlInit(void)
{
	...
	//外环控制参数初始化
	if(UI.RunMode==CV) //电压左外环的参数
	{
		OutLoopPI.Kp =0.02;
		OutLoopPI.Ki =0.0001;
		OutLoopPI.Sum =0;
		OutLoopPI.LowLimit =-10;
		OutLoopPI.UpLimit =10;
	}
	else //电流做外环的参数
	{
		OutLoopPI.Kp =0.05;
...
void Control(void)
{
	...
	if(UI.DIR==L2R)
	{
		I_L =-Meter.Value[IL]; //电感电流
	   Uo =Meter.Value[UR];   //输出电压
		Io =Meter.Value[RIO];  //输出电流
	}
	else
	{
		I_L =Meter.Value[IL];
	...
	//通过计算外环得到电流内环的指令值
	{
		if(UI.RunMode==CV) //外环是电压环
		{
			OutLoopPI.Reference =UI.OutLoopSet;
			OutLoopPI.FeedBack =Uo; //反馈是电压
			RunPI(&OutLoopPI);
			UI.SetI =OutLoopPI.Output;
		}
		else //外环是电流环
		{
			OutLoopPI.Reference =UI.OutLoopSet;
			OutLoopPI.FeedBack =Io;//反馈是输出电流
			RunPI(&OutLoopPI);
			UI.SetI =OutLoopPI.Output;
		}
	}
   //运行电流内环
	{
		if(UI.DIR==R2L)//电感电流与占空比正相关,正常控制
		{
			CurrentPI.Reference =UI.SetI;
			CurrentPI.FeedBack =I_L;
		}
		else //电感电流与占空比负相关,因此控制环路上要加一个负号
		{
			//通过指令与反馈交换实现加负号
		...
		PWM(CurrentPI.Output); //刷新占空比输出
	}
	
	//输出示波器观察
	JScope.Value1 =(int16_t)(OutLoopPI.Reference*100);
	JScope.Value2 =(int16_t)(OutLoopPI.FeedBack*100);
	JScope.Value3 =(int16_t)(OutLoopPI.Error*100);
